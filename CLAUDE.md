...
**Phase 5 — Anki export. Every build item is complete.** The whole 2,526-episode capture has now been audited (2026-08-11), Resolution is **verified live as of 2026-08-13** (checklist Section A passed in full) and offline-clean: eight defects fixed, and a full 2,526-episode audit reports 0 COLLISION / 0 DUPLICATE / 0 EMPTY with the `--background` proof still separating old code (9 bugs) from current (0). The edit-panel/chip and audio-capture batches from 2026-07-31 are fixed but **still never re-tested**. Details in `project-plan.md` Section 6 Phase 5, which opens with a **"Follow-up plan — start here next session"** block listing all five. Next, in priority order:
1. **Re-test the 2026-07-31 fixes live** (checklist groups B–D) plus the 20–30 minute continuous-session check. **Now the only unverified block in the project** — resolution's Section A passed in full on 2026-08-13.
2. **Fix the rest of the 2026-07-31 report** — the subtitle-text filtering group and the NanakoRaws line-break shape.
3. Sentence-only capture remains deliberately unscheduled — it needs real usage data first (see Build Order).
