...
**Phase 5 — Anki export. Every build item is complete.** The whole 2,526-episode capture has now been audited (2026-08-11), Resolution is offline-clean as of 2026-08-12: eight defects fixed, and a full 2,526-episode audit reports 0 COLLISION / 0 DUPLICATE / 0 EMPTY with the `--background` proof still separating old code (9 bugs) from current (0). The edit-panel/chip and audio-capture batches from 2026-07-31 are fixed but **still never re-tested**. Details in `project-plan.md` Section 6 Phase 5, which opens with a **"Follow-up plan — start here next session"** block listing all five. Next, in priority order:
...
3. Resolution needs a **fresh live pass** — checklist Section A was rewritten 2026-08-12 for the eight fixes; offline cleanliness cannot show that subtitles appear on screen.
