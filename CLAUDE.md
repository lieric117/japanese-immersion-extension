...
1. **Build the two 2026-09-22 caption decisions** — sign cues never pair as translations (next session), then no audio clip on non-Japanese audio; scope in `project-plan.md` Section 6 Phase 5.
2. **Live-test the 2026-08-15 work and the 2026-09-18/20 correctness audit** — checklist groups B–F (F8 passed 2026-09-22), plus the 20–30 minute continuous-session check. The audit is written up in `docs/audit/`.
3. **Settle the NanakoRaws line-break shape** — the rest of the 2026-07-31 filtering report was fixed by the audit.
4. Sentence-only capture stays deliberately unscheduled until there's real usage data (see Open Questions).
...
**Correctness audit harness (`scripts/audit/`, see `docs/audit/progress.md`):** `detection-sweep.js` and `parse-sweep.js` replay live Jimaku responses cached in `.audit-cache/` (`JIMAKU_CACHE_MODE=offline`), so a before/after comparison runs on identical data; `audit-resolution.js`/`sweep-resolution.js` now use the same cache.

Offline tests live in `scripts/`: `batch-test.js` (corpus baseline `92/49/57/33/354/109/9/9/35/2` — Pattern 2 rose from 39 on 2026-09-18 because grunts now miss honestly instead of resolving to unrelated verbs), `test-parse-invariants.js` (property test), `test-subtitle-parser.js`, `test-display-join.js`, `test-utterance-lemmas.js`, `test-detection-identity.js`, `test-subtitle-binding.js`, `test-detect-show-episode.js`, `test-stale-timeout.js`, `test-remembered-entry.js`, `test-named-season.js`, `test-fractional-special.js` (the last three replay recorded live responses from `fixtures/jimaku/`), `test-display-filters.js`, `test-entry-resolution.js`, `test-sibling-sniffer.js`, `test-season-resolution.js`, `test-english-bridging.js`, `test-edit-last-card.js`, `test-edit-panel.js`, `test-render-pipeline.js`, `test-merged-audio-span.js` (`--live` for the real-file replay). `collect-crunchyroll-fixtures.js`, `collect-jsonld-blocks.js` and `probe-play-captions.js` (every caption track in Crunchyroll's `play` response) are NOT node scripts — they're pasted into a logged-in browser console, the first to bulk-collect catalogue data, the second to capture real `TVEpisode` blocks and how fast they follow a navigation (see their headers); `analyze-crunchyroll-fixtures.js <collected.json>` sweeps that capture for metadata shapes only; `audit-resolution.js <capture.json>` is the one that tests resolution. Captures live in `fixtures/`.
...
`JIMAKU_API_KEY` is a persistent environment variable in `~/.zshenv` (re-created 2026-09-20 after the file went missing). The Bash tool's shell doesn't source it: run live scripts as `zsh -c 'source ~/.zshenv; node …'`.
